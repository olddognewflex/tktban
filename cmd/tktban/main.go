...
func run(argv []string) int {
	...
	cfg, settingsPath := *config, ""
	if *inHerdr {
		cwd, _ := os.Getwd()
		cfg, settingsPath = herdrSetup(cfg, os.Getenv, cwd, os.Stat)
	}
...
// herdrSetup applies herdr plugin mode and returns the tkt config path and the
// settings path to use ("" means the standalone default for each). An explicit
// --config always wins; else the config is found from the herdr context
// (focused pane, then workspace, then cwd). Settings move to the plugin state
// dir. Outside herdr (HERDR_ENV unset) nothing changes.
func herdrSetup(config string, getenv func(string) string, cwd string, stat func(string) (fs.FileInfo, error)) (string, string) {
	...
	return config, herdr.SettingsPath(e)
}
